...
def test_calc_energy(calc_chunk):
    ...
    assert (
        np.abs(calc_chunk.atoms.get_potential_energy() - free_energy) < eps_hp
    )
...
def test_molecular_calc_energy(molecular_calc_chunk):
    ...
    assert (
        np.abs(molecular_calc_chunk.atoms.get_potential_energy() - free_energy)
        < eps_hp
    )
    assert (
        np.abs(molecular_calc_chunk.results['total_energy'] - total_energy)
        < eps_hp
    )
...
def test_molecular_spin_calc_eigenvalues(molecular_spin_calc_chunk):
    ...
    assert np.allclose(
        molecular_spin_calc_chunk.eigenvalues[0, :, :], eigenvalues
    )
    assert np.allclose(
...
def test_molecular_spin_calc_occupancies(molecular_spin_calc_chunk):
    ...
    assert np.allclose(
        molecular_spin_calc_chunk.occupancies[0, :, :], occupancies
    )
    assert np.allclose(
