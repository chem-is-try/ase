...
def read_gpaw_out(fileobj, index):  # -> Union[Atoms, List[Atoms]]:
    ...
    images = []
    for i, block in enumerate(find_blocks(lines)):
        # XXX wrong to pass the charge here, it could differ in each block!
        atoms = process_block(block, q, is_first=not images)
        if atoms is None:
            break
